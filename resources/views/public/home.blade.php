        ...
        <div class="forum-container">
            ...
            <div class="row">
                ...
                <div class="forum-sidebar col-md-3">
                    <a class="forum-sidebar-create-button"
                       ...
                       class="btn btn-primary btn-block"><i class="forum-new"></i> @lang('New discussion')</a>
                        ...
                        <ul class="forum-sidebar-category-list">
                            ...
                            @foreach ($categories as $category)
                                <li class="forum-sidebar-category-item">
                                    <a class="forum-sidebar-category-item-link {{ isset($currentCategory) && $category->slug === $currentCategory->slug ? 'active' : '' }}"
                            ...
                            @foreach ($discussions as $discussion)
                                <li class="forum-discussion">
                                    <a class="forum-discussion-link" href="{{ route($lang.'::forum.discussion.showInCategory', [$discussion->category->slug, $discussion->slug]) }}">
                                        <div class="forum-discussion-avatar"
                                             style="background-color:#{{ ForumHelper::stringToColorCode($discussion->user->first_name) }}">
                                            {{ strtoupper(substr($discussion->user->first_name, 0, 1)) }}
                                        </div>
                                        <div class="forum-discussion-content">
                                            <h2 class="forum-discussion-content-title">{{ $discussion->title }}
                                                <div class="forum-discussion-content-category" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</div>
                                            </h2>
                                            <div
                                                class="forum-discussion-content-info">@lang('Posted by') {{ $discussion->user->first_name.' '.$discussion->user->last_name }} {{ Carbon::createFromTimeStamp(strtotime($discussion->created_at))->diffForHumans() }}</div>
                                            @if ($firstPost = $discussion->posts->first() and $firstPost !== null)
                                                <p class="forum-discussion-content-text">{{ substr(strip_tags($firstPost->body), 0, 200) }}@if (strlen(strip_tags($firstPost->body)) > 200)
                                                        {{ '…' }}
                                                    @endif</p>
                                            @endif
                                        </div>
